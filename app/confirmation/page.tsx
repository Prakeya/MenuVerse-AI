...
export default function ConfirmationPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ textAlign: 'center', position: 'relative', zIndex: 1, maxWidth: 420, width: '100%' }}>
        ...
        <h1 style={{ fontSize: 32, fontWeight: 800, color: 'white', margin: '0 0 8px' }}>{t('Order Confirmed!')}</h1>
        <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 15, margin: '0 0 32px' }}>{t('Sit back — your kitchen is already on it.')}</p>
        ...
        <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 20, marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', letterSpacing: '2px', marginBottom: 8 }}>{t('ESTIMATED PREP TIME')}</div>
        ...
        {/* Live Status */}
        <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 20, marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', letterSpacing: '2px', marginBottom: 16 }}>{t('LIVE STATUS')}</div>
          {[
            { label: t('Order Confirmed'), sub: "We've received your order", done: true },
            { label: t('Being Prepared'), sub: 'The kitchen is on it', done: timeLeft < 800 },
            { label: t('Ready to Serve'), sub: 'Your food is on its way', done: timeLeft < 100 },
          ].map((step, i) => (
        ...
        <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 20, marginBottom: 24 }}>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', letterSpacing: '2px', marginBottom: 12 }}>{t('YOUR ORDER')}</div>
          {orderItems.length > 0 ? (
            <>
               {orderItems.map((item, idx) => {
                const tr = translateMenuItem(item.item.item_id, item.item.name, item.item.description)
                return (
                <div key={idx} style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 4 }}>
                  {item.quantity} x {tr.name} — ${(item.item.price * item.quantity).toFixed(2)}
                </div>
              )})}
            ...
            <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>{t('No items')}</div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <button 
            ...
            style={{ width: '100%', padding: '16px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'white', fontSize: 15, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
          >
            <ShoppingBag size={18} />
          ...
          <button 
            onClick={() => router.push('/menu/demo')} 
            style={{ width: '100%', padding: '16px', borderRadius: 999, border: 'none', background: '#F5A623', color: '#111', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
          >
            <ArrowLeft size={18} />
            {t('Back to Menu')}
          </button>
