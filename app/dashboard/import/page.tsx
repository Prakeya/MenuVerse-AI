  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 800, color: '#1a1a1a', margin: 0 }}>{t('Magic Menu Import')}</h1>
        <p style={{ fontSize: 14, color: '#6b7280', margin: '4px 0 0' }}>{t('Paste or upload your existing menu — our AI extracts every dish instantly.')}</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a' }}>{t('Paste your menu text')}</label>
            <button onClick={loadExample} style={{ background: 'none', border: 'none', color: '#F5A623', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>{t('Load example')}</button>
          </div>
          ...
          <div style={{ marginTop: 16, border: '2px dashed #e5e7eb', borderRadius: 16, padding: 32, textAlign: 'center', cursor: 'pointer', transition: 'all 0.15s', background: '#f9fafb' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: '#1a1a1a' }}>{t('Drop a .txt or .csv file')}</div>
            <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>{t('or click to browse')}</div>
          </div>
          ...
          <button onClick={() => { if (text.trim()) setLoading(true) }} style={{ marginTop: 16, width: '100%', padding: '14px', borderRadius: 999, border: 'none', background: text.trim() ? '#111' : '#e5e7eb', color: text.trim() ? 'white' : '#9ca3af', fontSize: 15, fontWeight: 700, cursor: text.trim() ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ✦ {t('Import Menu with AI')}
          </button>
        </div>

        <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 20, padding: 24 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#1a1a1a', marginBottom: 16 }}>{t('Import Log')}</div>
          {loading ? (
            ...
            <div style={{ padding: 32, textAlign: 'center', color: '#6b7280', fontSize: 13 }}>{t('Import something to see the log')}</div>
          )}
        </div>
      </div>
    </div>
