...
export default function MenuStudioPage() {
  ...
  const [addForm, setAddForm] = useState({ name: '', description: '', price: 0, category: '', ingredients: '', allergens: '' })
  ...
  const saveEdit = async () => {
    if (!editing) return
    try {
      const res = await fetch(`/api/menu/rest_1/${editing.item_id}`, {
        method: 'PUT',
  ...
  const toggleAvail = async (id: string) => {
    ...
    if (!dish) return
    try {
      const res = await fetch(`/api/menu/rest_1/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const openAdd = () => {
    setAddForm({ name: '', description: '', price: 0, category: CATEGORIES[0] || '', ingredients: '', allergens: '' })
    setAdding(true)
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <button onClick={openAdd} style={{ padding: '12px 24px', borderRadius: 999, border: 'none', background: '#111', color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>+ {t('Add Dish')}</button>
      </div>

      {loading ? (
        <div style={{ textAlign: 'center', padding: 40, color: '#6b7280' }}>Loading menu...</div>
      ) : (
        <>
      ...
      {/* Add Dish Modal */}
      {adding && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setAdding(false)}>
          <div style={{ background: 'white', borderRadius: 20, padding: 24, width: 500, maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{t('Add Dish')}</h3>
              <button onClick={() => setAdding(false)} style={{ width: 32, height: 32, borderRadius: '50%', border: '1px solid #e5e7eb', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={16} /></button>
            </div>
            <div style={{ display: 'grid', gap: 16 }}>
              {[
                ...
                <div key={f.key}>
                  ...
                  {f.textarea ? (
                    <textarea value={addForm[f.key]} onChange={e => setAddForm({ ...addForm, [f.key]: e.target.value })} style={{ width: '100%', padding: '10px 14px', borderRadius: 12, border: '1px solid #e5e7eb', fontSize: 13, outline: 'none', minHeight: 80, resize: 'vertical' }} />
                  ) : (
                    <input type={f.type || 'text'} value={addForm[f.key]} onChange={e => setAddForm({ ...addForm, [f.key]: f.key === 'price' ? parseFloat(e.target.value) || 0 : e.target.value })} style={{ width: '100%', padding: '10px 14px', borderRadius: 12, border: '1px solid #e5e7eb', fontSize: 13, outline: 'none' }} />
                  )}
                </div>
              ))}
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a', display: 'block', marginBottom: 6 }}>{t('Ingredients (comma-separated)')}</label>
                <input value={addForm.ingredients} onChange={e => setAddForm({ ...addForm, ingredients: e.target.value })} style={{ width: '100%', padding: '10px 14px', borderRadius: 12, border: '1px solid #e5e7eb', fontSize: 13, outline: 'none' }} />
              </div>
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a', display: 'block', marginBottom: 6 }}>{t('Allergens (comma-separated)')}</label>
                <input value={addForm.allergens} onChange={e => setAddForm({ ...addForm, allergens: e.target.value })} style={{ width: '100%', padding: '10px 14px', borderRadius: 12, border: '1px solid #e5e7eb', fontSize: 13, outline: 'none' }} />
              </div>
              <button onClick={saveAdd} style={{ marginTop: 8, padding: '12px 24px', borderRadius: 999, border: 'none', background: '#111', color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <Save size={16} /> {t('Add Dish')}
              </button>
            </div>
          </div>
        </div>
