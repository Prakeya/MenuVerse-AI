...
export default function MenuPage() {
  ...
  const fetchMenu = async () => {
    try {
      const res = await fetch('/api/menu/demo')
      if (res.ok) {
  ...
  const translatedHero = hero ? translateMenuItem(hero.item_id, hero.name, hero.description) : { name: '', description: '' }
