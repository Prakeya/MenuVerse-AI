...
import { ArrowLeft, MapPin, Globe, ChevronDown, ShoppingCart, MessageCircle, Flame, Star, Heart, ChefHat, AlertTriangle, Ban } from 'lucide-react'
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showLang && (
                <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, background: 'white', borderRadius: 16, boxShadow: '0 12px 32px rgba(0,0,0,0.1)', padding: 8, minWidth: 160, zIndex: 20 }}>
                  {['English', 'Spanish', 'French', 'Hindi'].map(l => (
                    <button key={l} onClick={() => setShowLang(false)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 12px', borderRadius: 8, border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 13, color: '#1a1a1a' }}>{l}</button>
                  ))}
                </div>
              )}
            </div>
