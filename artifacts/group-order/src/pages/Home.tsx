...
import { Building2, Users, Globe, ChevronDown } from 'lucide-react'
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', inset: 0 }}>
        ...
        />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(0,0,0,0.4), rgba(10,10,10,0.92))' }} />
      </div>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ position: 'relative' }}>
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 999, padding: '8px 14px', color: 'white', cursor: 'pointer', fontSize: 13 }}
            >
              <Globe size={15} />
            ...
            {showLang && (
              <div style={{ position: 'absolute', top: '110%', right: 0, background: 'white', borderRadius: 14, boxShadow: '0 12px 32px rgba(0,0,0,0.2)', padding: 8, minWidth: 150, zIndex: 20 }}>
                {LANGUAGES.map((l) => (
                  <button
                    key={l.code}
                    onClick={() => { setLang(l.code); setShowLang(false) }}
                    style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 12px', borderRadius: 8, border: 'none', background: lang === l.code ? '#f5f5f5' : 'transparent', cursor: 'pointer', fontSize: 13, color: '#1a1a1a' }}
                  >
                    {l.name} {lang === l.code && '✓'}
                  </button>
                ))}
        ...
        <div style={{ maxWidth: 700, textAlign: 'center' }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 999, padding: '6px 16px 6px 6px', marginBottom: 32 }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>
              2,400+ restaurants join the table.{' '}
              <span style={{ color: 'white' }}>Join us.</span>
            </span>
          </div>

          <h1 style={{ fontSize: 'clamp(40px, 8vw, 80px)', fontWeight: 800, lineHeight: 1.05, letterSpacing: '-3px', color: 'white', margin: '0 0 20px' }}>
            Taste.{' '}
            <span style={{ color: '#F5A623' }}>Order.</span>
            <br />
            Delight.
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 16, maxWidth: 480, margin: '0 auto 40px', lineHeight: 1.7 }}>
            AI-powered restaurant OS that turns every table into a cinematic dining experience.
          </p>

          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link
              href="/menu/restaurant_001"
              style={{ background: '#F5A623', color: '#111', borderRadius: 999, padding: '16px 32px', fontSize: 16, fontWeight: 700, cursor: 'pointer', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 10 }}
            >
              <Users size={20} />
              I'm a Diner
            </Link>
            <Link
              href="/signin"
              style={{ background: 'rgba(255,255,255,0.08)', color: 'white', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 999, padding: '16px 32px', fontSize: 16, fontWeight: 600, cursor: 'pointer', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 10 }}
            >
              <Building2 size={20} />
              I'm an Owner
            </Link>
      ...
      {/* Stats bar */}
      <div style={{ position: 'relative', zIndex: 10, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(12px)', borderTop: '1px solid rgba(255,255,255,0.08)', padding: '20px 32px' }}>
